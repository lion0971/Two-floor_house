...
const SLIDE_SIGN_OVERRIDE = {
  // 'sliding_door_名稱': 1,  // 預設 -1（往負方向開），需要往正方向開就填 1
};
...
loader.load(CONFIG.MODELS.BUILDING, (gltf) => {
  ...
  gltf.scene.traverse((mesh) => {
    ...
    if (name.includes('sliding_door')) {
      ...
      // ── 自動判斷滑動軸向：比較本地包圍盒 X / Z 哪個較長 ──
      // 門板較寬的那個軸，通常就是它實際要沿著滑軌平移的方向
      // 若自動判斷不準，可在 SLIDE_AXIS_OVERRIDE / SLIDE_SIGN_OVERRIDE 手動指定特定門
      const slideAxis = SLIDE_AXIS_OVERRIDE[name]
        ?? (size.x >= size.z ? 'x' : 'z');
      const DOOR_OPEN_MARGIN = 0.1; // 開門時保留 10 公分不滑出去，方便點擊
      const slideDistance = Math.max(0, (slideAxis === 'z' ? size.z : size.x) - DOOR_OPEN_MARGIN);
      const slideSign = SLIDE_SIGN_OVERRIDE[name] ?? -1; // 預設往負方向開，特定門可覆寫成 +1
      ...
      doorAnimations[name] = {
        ...
        type: 'sliding',
        slideAxis,                 // ← 新增：'x' 或 'z'
        slideSign,                 // ← 新增：開門方向 +1 / -1
        slideDistance,             // 依實際滑動軸的寬度計算
        startX: mesh.position.x,   // 兩個軸的初始位置都先記錄起來
        startZ: mesh.position.z,
      };
    } else if (name.includes('door_')) {
      doorObjects.push(mesh);
      doorAnimations[name] = {
        mesh,
        isOpen: false,

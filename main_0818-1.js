...
function tryShowTapPrompt() {
  if (isLoadingFinished && window.__homeOverlayClosed === true) {
    showTapPrompt();
  }
}

// 遮罩比載入動畫晚關閉時，靠這個事件監聽補上觸發
document.addEventListener('homeOverlayClosed', tryShowTapPrompt);

function finishLoading() {
  applyLeafStartFromLoadingIcon(); // ← 新增這一行，必須在 loading 畫面隱藏前執行
...
// ── 量測 loading 畫面裡的葉片，讓提示動畫的葉片起始大小/位置對齊它 ──
const loadingLeafEl = document.querySelector('.leaf-icon');

function applyLeafStartFromLoadingIcon() {
  if (!loadingLeafEl) return;

  const loadingRect = loadingLeafEl.getBoundingClientRect();

  // 防呆：量到 0 代表父層已隱藏或尚未 layout 完成，直接放棄，退回預設十字標位置
  if (loadingRect.width === 0 && loadingRect.height === 0) {
    console.warn('[TapPrompt] 量測 .leaf-icon 失敗，略過起始位置對齊');
    return;
  }

  const loadingFontSize = parseFloat(getComputedStyle(loadingLeafEl).fontSize) || 48;
  const startScale = loadingFontSize / 48;

  const startX = loadingRect.left + loadingRect.width / 2;
  const startY = loadingRect.top + loadingRect.height / 2;

  tapPrompt.style.transition = 'none';
  tapPrompt.style.left = startX + 'px';
  tapPrompt.style.top = startY + 'px';
  tapPromptLeaf.style.setProperty('--leaf-start-scale', startScale.toFixed(3));

  void tapPrompt.offsetWidth; // 強制 reflow

  requestAnimationFrame(() => {
    tapPrompt.style.transition = 'opacity 0.4s ease, left 1.2s cubic-bezier(0.55,0,0.85,0.35), top 1.2s cubic-bezier(0.55,0,0.85,0.35)';
    updateTapPromptPosition(); // 移動到十字標座標
  });
}

function updateTapPromptPosition() {
  const { x, y } = getCrosshairCenter();
  tapPrompt.style.left = x + 'px';
  tapPrompt.style.top = y + 'px';
}

window.addEventListener('resize', () => {
  if (tapPrompt.style.display === 'flex') {
    updateTapPromptPosition();
  }
});
...
// ── 點擊提示文字：載入完成後顯示，第一次點擊/觸碰後消失 ──
// ── 點擊提示：葉片飄落 + 水波紋 + 文字，全部用 CSS 動畫（不進 render loop，成本可忽略）──
// ── 點擊提示：葉片飄落 + 水波紋 + 文字，全部用 CSS 動畫（不進 render loop，成本可忽略）──
...
@keyframes tapLeafFall {
  0%   { transform: scale(var(--leaf-start-scale, 1.7)); opacity: 0; }
  10%  { transform: scale(var(--leaf-start-scale, 1.7)); opacity: 1; }
  100% { transform: scale(0.34); opacity: 1; }
}

/* 落地後：維持最小尺寸，只做輕微左右擺動＋小角度旋轉，無限循環 */
@keyframes tapLeafSway {
  0%, 100% { transform: scale(0.34) rotate(-4deg) translateX(-2px); }
  50%      { transform: scale(0.34) rotate(4deg)  translateX(2px); }
}

/* 水波紋：正圓形（寬高相等 + border-radius:50%），從落地那一刻開始持續循環展開 */
...
/* 掉落中：只播放一次 */
.tap-prompt-leaf.falling {
  animation: tapLeafFall 1.2s cubic-bezier(0.55, 0, 0.85, 0.35) forwards;
  animation-iteration-count: 1;
}
/* 落地後：無限輕微晃動 */
.tap-prompt-leaf.landed {
  opacity: 1;
  animation: tapLeafSway 2.6s ease-in-out infinite;
}
...
// ⚡ 葉片掉落動畫播完的那一刻，才切換成「落地晃動」+ 顯示水波紋 + 淡入文字
// ⚡ 葉片掉落動畫播完的那一刻（縮到最小、落地），才切換成「落地晃動」
// + 顯示水波紋 + 淡入文字 + 黑幕開始淡出消失
tapPromptLeaf.addEventListener('animationend', (e) => {
  if (e.animationName !== 'tapLeafFall') return;
  tapPromptLeaf.classList.remove('falling');
  tapPromptLeaf.classList.add('landed');
...
function showTapPrompt() {
  // ⚡ 不在這裡呼叫 applyLeafStartFromLoadingIcon()，它已經在 finishLoading() 裡執行過了
  tapPromptLeaf.classList.remove('landed');
  tapPromptRipple.classList.remove('rippling');
  tapPromptText.style.opacity = '0';
  tapPromptLeaf.style.animation = 'none';
  void tapPromptLeaf.offsetWidth;
  tapPromptLeaf.style.animation = '';
  tapPromptLeaf.classList.add('falling');
...
function hideTapPrompt() {
  ...
  setTimeout(() => {
    ...
    tapPromptLeaf.classList.remove('falling', 'landed');
    tapPromptRipple.classList.remove('rippling');
  }, 400);
}
...
let blackCoverFaded = false; // 避免重複觸發（多次呼叫 fadeOutBlackCover 也不會出錯）

...
function checkCurrentCollision() {
  ...
  for (const dir of rayDirections) {
    // 原本：dir.clone().applyQuaternion(...).normalize() 每次都會 new 一個 Vector3
    _tmpWorldDir.copy(dir).applyQuaternion(camera.quaternion).normalize();
    raycaster.set(camera.position, _tmpWorldDir);
...
/**
 * 共用的碰撞判斷：只在「膝蓋附近高度」（原三高度裡最下面那個）左右各開一個肩寬偵測點（共 2 條射線）。
 * 這組偵測點打射線，若擋到就修改 moveVelocity（沿牆滑動）。
 * 一般模式跟透視模式都呼叫這個函式，只差在傳入的物件清單不同，
 * 確保「偵測點邏輯」永遠只有一套，不會分岔成兩份。
 *
 * ⚠️ 取捨提醒：原本的 COLLISION_HEIGHT_OFFSETS 多高度設計是為了解決
 * 「牆壁在某個高度剛好是窗戶鏤空範圍」導致穿牆的問題（跟這裡的接縫縫隙
 * 是兩種不同成因的破洞）。精簡成只剩單一高度後，如果之後又發現在窗戶
 * 附近能穿牆，很可能就是這個舊 bug 重新出現，屆時把下面的 yOffset 迴圈
 * 加回來即可（保留在下方註解，供還原使用）。
 */
/**
 * 共用的多高度碰撞判斷：對 targetObjects 用 COLLISION_HEIGHT_OFFSETS × 左右肩寬偵測點。
 * 這組偵測點打射線，若擋到就修改 moveVelocity（沿牆滑動）。
 * 一般模式跟透視模式都呼叫這個函式，只差在傳入的物件清單不同，
 * 確保「偵測點邏輯」永遠只有一套，不會分岔成兩份。
 *
 * ⚠️ 曾經試過精簡成「只用膝蓋高度」，但實測發現有些牆體的破口剛好只存在於
...
function resolveCollisionSlide(moveVelocity, targetObjects) {
  ...
  for (const yOffset of COLLISION_HEIGHT_OFFSETS) {
...
function handleMovementAndCollision(moveVelocity) {
  ...
  resolveStaircaseCylinderCollision(moveVelocity); // ⚡ 新增：樓梯除入口外的圓柱阻擋
  ...
  gltf.scene.traverse((obj) => {
    ...
    if (isTurnUpDoor) {
      ...
      doorAnimations[name] = {
        ...
        turnUpAxis: TURN_UP_AXIS_OVERRIDE[name] ?? 'x', // 預設繞 X 軸，可用 override 改成 y 或 z

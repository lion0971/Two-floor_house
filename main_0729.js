...
const STRICT_COLLISION_NAMES = new Set([
  'wall_outside_2',
  'livingroom_big_w_1_wall.002',
  'livingroom_big_w_wall.002',
  'livingroom_big_w_1_wall.005',
  'livingroom_big_w_wall.001',
]);
...
function handleMovementAndCollision(moveVelocity) {
  if (isNoclipMode) {
    ...
    // 避免走出建築外牆、走到半空中。
    if (strictCollidableObjects.length > 0 && (moveVelocity.x !== 0 || moveVelocity.z !== 0)) {
      const moveDir = moveVelocity.clone().normalize();
      raycaster.set(camera.position, moveDir);
      const intersects = raycaster.intersectObjects(strictCollidableObjects);

...
// ⚡ 修正「室內地板光影會隨太陽位置移動、無視天花板遮蔽」的問題：
// 原本完全沒開陰影系統，太陽光直接穿透天花板、牆壁照到地板，
// 沒有任何遮蔽計算。開啟 shadowMap 後，還要在下面幫 sunLight 開
// castShadow，並幫天花板/牆壁/地板等 mesh 分別設 castShadow/receiveShadow
// 才會真正生效（見 gltf.scene 第二遍 traverse 跟 sunLight 設定處）。
renderer.shadowMap.enabled = true;
renderer.shadowMap.type = THREE.PCFSoftShadowMap; // 邊緣柔化，比預設 PCFShadowMap 更自然
...
loader.load(CONFIG.MODELS.BUILDING, (gltf) => {
  ...
  // ⚡ 陰影攝影機（sunLight.shadow.camera）建立時是用猜測的 ±20 範圍、
  // 目標對準原點（0,0,0）——但建築實際位置未必以原點為中心。
  // 這裡用模型真正的世界座標邊界框，重新校正陰影攝影機的目標跟範圍，
  // 確保不管建築實際蓋在哪裡，陰影都完整覆蓋整個空間，不會被裁掉一角。
  {
    ...
    const halfExtent = Math.max(modelSize.x, modelSize.z) * 0.6; // 留一點餘裕
    ...
    sunLight.shadow.camera.far = 60 /* 太陽距離 dist */ + Math.max(modelSize.x, modelSize.y, modelSize.z) + 20;
  ...
  gltf.scene.traverse((mesh) => {
    ...
    // ⚡ 讓天花板/牆壁能真正擋住太陽光、地板能接收陰影：
    // 每個實體 mesh 預設都同時投射+接收陰影，這樣不管是哪個名稱、
    // 之後模型再新增什麼物件，遮光關係都會自動正確，不用一個個列名單。
    // 半透明的玻璃/水面之類如果之後發現陰影效果不自然，可以再針對
    // 特定名稱關閉（例如水面通常不需要 castShadow）。
    ...
    if (Object.values(PIPE_CONFIG).some(cfg => cfg.drainKey === name)) {
      ...
      const drainRadius = name.includes('shower') ? 0.6 : 0.2;
...
// ⚡ 開啟陰影投射：沒有這行，天花板/牆壁不會擋光，室內地板的光影
// 就會直接跟著太陽方向同步移動，完全無視上方有沒有遮蔽物。
sunLight.castShadow = true;
sunLight.shadow.mapSize.set(2048, 2048);   // 解析度，數字越大陰影邊緣越銳利，效能成本也越高
sunLight.shadow.bias = -0.0015;             // 避免陰影錠紋(shadow acne)，數字太大則會出現漏光(peter-panning)
sunLight.shadow.camera.near = 1;
sunLight.shadow.camera.far = 150;           // 太陽距離設 60，這裡留足margin
// DirectionalLight 用正交攝影機投影陰影，範圍要能覆蓋整個室內空間，
// 依目前建築大致尺寸先給一個寬鬆值，若邊緣陰影被裁掉可再加大。

...
const FILTER_RING_ZONES = [
  { min: 0, max: 25, color: '#3b82f6', dim: 'rgba(59,130,246,0.25)' },  // 藍
  { min: 25, max: 50, color: '#22c55e', dim: 'rgba(34,197,94,0.25)' },   // 綠
  { min: 50, max: 75, color: '#eab308', dim: 'rgba(234,179,8,0.25)' },   // 黃
  { min: 75, max: 90, color: '#f97316', dim: 'rgba(249,115,22,0.25)' },  // 橘
  { min: 90, max: 100, color: '#ef4444', dim: 'rgba(239,68,68,0.25)' },   // 紅
];

// ── 卡片顯示的距離/視野門檻 ──
const FILTER_CARD_MAX_DISTANCE = 10.0;         // 公尺：超過這個距離直接不顯示，省去DOM更新與排版計算
const FILTER_CARD_VISIBILITY_INTERVAL = 0.15; // 距離/視野判斷節流間隔（秒），不需要每一幀都算
...
function createFilterCard(device) {
  ...
  Object.assign(root.style, {
    ...
    left: '50%',
    bottom: '0',
    marginLeft: '-34px',  // 抵銷圓環寬度(約68px)的一半，橫向置中對齊錠點
    marginBottom: '10px',
    opacity: '0',
  ...
  const zoneCircles = FILTER_RING_ZONES.map(zone => {
    ...
    circle.setAttribute('cx', '30'); circle.setAttribute('cy', '30'); circle.setAttribute('r', String(RING_RADIUS));
    circle.setAttribute('fill', 'none');
    circle.setAttribute('stroke-width', '6');
    circle.setAttribute('stroke-linecap', 'butt'); // 區塊間不要圓角，交界處才會對齊
    circle.setAttribute('stroke-dasharray', `${segLen} ${RING_CIRC - segLen}`);
    circle.setAttribute('stroke-dashoffset', String(dashOffset));
    circle.setAttribute('transform', 'rotate(-90 30 30)'); // 從12點鐘方向開始，跟原本一致
    circle.setAttribute('stroke', zone.dim); // 預設先畫暗色，updateFilterUI 會依目前用量點亮對應區塊
  ...
  filterUICards[device] = {
    ...
    zoneCircles, hideTimer: null, isVisible: false,
  };
}
...
function updateFilterUI(device) {
  ...
  card.zoneCircles.forEach((circle, i) => {
    const zone = FILTER_RING_ZONES[i];
    const isLast = i === FILTER_RING_ZONES.length - 1;
    const inZone = usedPercent >= zone.min && (isLast ? usedPercent <= zone.max : usedPercent < zone.max);
    circle.setAttribute('stroke', inZone ? zone.color : zone.dim);
  });
}
...
function updateFilterCardVisibility(device) {
  ...
  if (st.isActive) {
    ...
    const dist = camera.position.distanceTo(_filterCardWorldPos);

    if (dist <= FILTER_CARD_MAX_DISTANCE) {
...
function animate() {
  ...
  // ⚡ 節流：不需要每一幀都算距離跟是否在畫面內，每 FILTER_CARD_VISIBILITY_INTERVAL 秒算一次

...
const FILTER_RING_ZONES = [
  { min: 0, max: 25, color: '#60a5fa', dim: 'rgba(59,130,246,0.25)' },  // 藍（亮色也稍微提亮一階）
  { min: 25, max: 50, color: '#4ade80', dim: 'rgba(34,197,94,0.25)' },   // 綠
  { min: 50, max: 75, color: '#facc15', dim: 'rgba(24, 21, 9, 0.25)' },   // 黃
  { min: 75, max: 90, color: '#fb923c', dim: 'rgba(249,115,22,0.25)' },  // 橘
  { min: 90, max: 100, color: '#f87171', dim: 'rgba(239,68,68,0.25)' },   // 紅
];
...
// ── 卡片「遠近感」縮放設定 ──
// 距離 <= NEAR：維持現在的大小（scale = 1，跟原本一樣大）
// 距離 >= FAR：縮小到 MIN_SCALE，超過這個距離也不會再更小
const FILTER_CARD_SCALE_NEAR_DIST = 2.0;  // 公尺
const FILTER_CARD_SCALE_FAR_DIST = 12.0;  // 公尺
const FILTER_CARD_SCALE_MIN = 0.4;

// ⚡ 新增：把「水平避讓位移」跟「遠近縮放」合併套用到同一個 transform，
// 避免 resolveFilterCardOverlaps()（管位移）跟距離縮放各自覆寫 transform 互相打架。
function applyCardOffsetAndScale(card) {
  card.offsetWrapper.style.transform =
    `translateX(${card.offsetX}px) scale(${card.distanceScale})`;
}
...
function createFilterCard(device) {
  ...
  const zoneCircles = FILTER_RING_ZONES.map(zone => {
    ...
    circle.setAttribute('transform', 'rotate(-90 40 40)'); // 從12點鐘方向開始，跟原本一致
  ...
  filterUICards[device] = {
    ...
    offsetX: 0,        // ⚡ 新增：水平避讓位移量
    distanceScale: 1,  // ⚡ 新增：依距離算出的縮放比例
  };
}
...
function updateFilterUI(device) {
  ...
  card.zoneCircles.forEach((circle, i) => {
    ...
    // ⚡ 用 CSS drop-shadow 做光暈，比 SVG feGaussianBlur+feMerge 更亮、更省（不用額外 DOM 節點）
    // ⚡ 目前所在區段：疊加飽和度+明度，讓它跟其他暗色區段的對比更明顯
    circle.style.filter = inZone
      ? `saturate(1.6) brightness(1.25) drop-shadow(0 0 4px ${zone.color}) drop-shadow(0 0 8px ${zone.color})`
      : 'none';
  });
}
...
function updateFilterCardVisibility(device) {
  ...
  if (st.isActive) {
    ...
    // ⚡ 遠近感：靠近＝維持現在的大小(1)，遠離＝依比例縮小到 FILTER_CARD_SCALE_MIN 為止
    const t = THREE.MathUtils.clamp(
      (dist - FILTER_CARD_SCALE_NEAR_DIST) / (FILTER_CARD_SCALE_FAR_DIST - FILTER_CARD_SCALE_NEAR_DIST),
      0, 1
    );
    card.distanceScale = THREE.MathUtils.lerp(1, FILTER_CARD_SCALE_MIN, t);

    // 不再限制距離，只看是否落在畫面（視野）範圍內

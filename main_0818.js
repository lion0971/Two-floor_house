...
function finishLoading() {
  ...
  setTimeout(() => {
    ...
    // 遮罩比載入動畫早關閉時（window.__homeOverlayClosed 已是 true），
    // 這裡直接就能觸發；若遮罩還沒關，這次呼叫會判斷失敗、不顯示，
    // 之後遮罩關閉時交給上面的事件監聽補觸發。
    tryShowTapPrompt();
  }, 600);
}
...
@keyframes tapLeafFall {
  ...
  10%  { transform: scale(1.6) translateY(0);      opacity: 1; }
  100% { transform: scale(0.34) translateY(0);     opacity: 1; }
}
...
// 葉片＋水波紋要疊在同一個相對定位區塊裡，水波紋才能貼齊葉片正下方
...
// 沿用 index.html 裡 <div class="leaf-icon">🌿</div> 同一顆葉子

...
/**
 * 共用的碰撞判斷：只在「膝蓋附近高度」（COLLISION_LATERAL_HEIGHT_OFFSET，
 * 即 COLLISION_HEIGHT_OFFSETS 三個高度裡最下面那個）左右各開一個肩寬偵測點，
 * 共 2 條射線。這組偵測點打射線，若擋到就修改 moveVelocity（沿牆滑動）。
...
function resolveCollisionSlide(moveVelocity, targetObjects) {
  ...
  for (let iter = 0; iter < MAX_SLIDE_ITERATIONS; iter++) {
    ...
    // ⚡ 只用最下面那個高度（膝蓋附近，COLLISION_LATERAL_HEIGHT_OFFSET）
    // 左右各開一個肩寬偵測點，共 2 條射線（原本是 3 個高度 × 左右 2 點＝6 條）。
  ...
  // 迭代次數用完仍持續碰撞：通常代表被夾在兩面牆的轉角/接縫處，
  // 殘留的滑動速度很可能同時朝著兩面牆推擠，這正是舊版「左右碰撞幾次後穿牆」
  // 的成因。直接歸零最安全，寧可讓玩家在轉角處卡住不動，也不要放行穿牆。

...
//      自動暫停動畫，沒人靠近時零成本。
//   3. 距離判斷：用便宜的「兩點距離」計算，而且節流到每
//      SWITCH_HINT_CHECK_INTERVAL 秒才算一次，不是每幀都算。
//   4. 數量：整個場景的開關就是 PIPE_CONFIG 列出的幾個裝置（目前8個），
//      距離檢查頂多8次減法+開根號，一秒跑幾次而已。
// ═══════════════════════════════════════════════════════════
...
const SWITCH_HINT_CHECK_INTERVAL = 0.2;     // 距離判斷節流間隔（秒），不需要每一幀都算
...
.switch-hint-fish-h {
  display: flex;
  align-items: center;
  animation: switchHintNibble 1s ease-in-out infinite;
  will-change: transform;
}
.switch-hint-fish-h.delayed {
  animation-delay: 0.5s; /* 跟另一條魚錯開半個週期，兩條魚不會同時前衝，看起來比較自然、不像機器 */
}
@keyframes switchHintNibble {
  0%, 100% { transform: translateX(0) scaleX(1); }
  45%      { transform: translateX(7px) scaleX(0.92); } /* 朝物件前衝一點，帶點壓扁模擬咬下去的力道 */

...
// ── 門把指示器（三角形＋泡泡）位置校正 ──
// 自動計算：doorWidth（門片寬度，來自geometry本地包圍盒）+ handleSign（門把在門軸的哪一側）。
// 如果自動抓出來的方向或距離不準，可在這裡針對特定門名稱手動覆寫 handleSign（1 或 -1）。

// ── 門把指示器「正面判斷」：只在使用者站在門的正面時才顯示三角形。
// 預設假設模型的門片正面法向量是本地 +Z 軸，如果實際上不對（三角形在
// 應該看不到的那一側還是顯示），針對該扇門填入正確的軸向/正負號即可。
const DOOR_FRONT_NORMAL_AXIS_OVERRIDE = {
  // 'door_livingroom': 'x',
};
const DOOR_FRONT_NORMAL_SIGN_OVERRIDE = {
  // 'door_livingroom': -1,
};
...
// 那本來就是合理可通過的開口，不應該擋。
const PLAYER_HALF_WIDTH = 0.2;//左右各 20 公分，总共约 40 公分宽
//碰撞宣告end
...
const STAIR_MID_Y = STAIRCASE.center.y + STAIRCASE.totalHeight / 2; // 計算樓梯的「中間高度」
...
// 常見的情況），都會把所有子孫 mesh 的頂點座標轉換到 target 的本地空間
// 再合併，這樣算出來的中心點/薄軸判斷，才能正確拿來當作 target 的
// 本地座標偏移使用（直接用世界座標包圍盒會混入 target 自己在世界中的
// 位置/朝向，數字完全對不上——這正是之前滑門跑位、正反面判斷亂掉的原因）。
// 呼叫時機必須在 gltf.scene.updateMatrixWorld(true) 執行過之後。
function computeDoorIndicatorMeta(target) {
  ...
  target.traverse((child) => {
    ...
    tmpBox.applyMatrix4(child.matrixWorld); // 子網格本地 → 世界座標
    tmpBox.applyMatrix4(invMatrix);          // 世界座標 → target 的本地座標
    box.union(tmpBox);
  });

  if (box.isEmpty()) {
    return { frontAxis: 'z', doorCenterX: 0, doorCenterY: 0 };
  }
  ...
  let frontAxis = 'z';
  if (sizeX <= sizeY && sizeX <= sizeZ) frontAxis = 'x';
  else if (sizeZ <= sizeX && sizeZ <= sizeY) frontAxis = 'z';
  else frontAxis = 'y';

  return {
    frontAxis,
    doorCenterX: (box.min.x + box.max.x) / 2,
    doorCenterY: (box.min.y + box.max.y) / 2,
  };
}

// ⚡ 依門的類型算出「本地座標系下」的開門方向單位向量，
// 給門靠近指示器（三角形＋泡泡）用來換算螢幕空間角度。
function getDoorOpenLocalDirection(anim) {
  if (!anim) return new THREE.Vector3(1, 0, 0);
  if (anim.type === 'sliding') {
    const sign = anim.slideSign ?? -1;
    if (anim.slideAxis === 'z') return new THREE.Vector3(0, 0, sign);
    if (anim.slideAxis === 'y') return new THREE.Vector3(0, sign, 0);
    return new THREE.Vector3(sign, 0, 0);
  }
  if (anim.type === 'turn_up') {
    return new THREE.Vector3(0, 1, 0); // 上掀式門固定指向上方
  }
  // ⚡ 旋轉門：指示器現在改掛在「門把」位置（門軸另一側、垂直置中），
  // 不再是貼在門軸正上方，所以「指回門軸」的方向也從垂直向下
  // 改成沿本地 X 軸水平指回原點（原點＝門軸）。
  const sign = anim.swingSign ?? -1;
  return new THREE.Vector3(-sign, 0, 0);
}
...
let stairEntryHeight = 0;   // ⚡ 新增：本次是從哪一端進入的（0=樓下，totalHeight=樓上），給 getStairAngleAtHeight() 判斷該往哪個目標角度做修正
...
function getStairAngleAtHeight(height) {
  ...
  // 理論公式在「終點」算出來的角度, 算出如果完全照數學公式走，走到底時會面對哪個方向
  const rawFarAngle = stairAngleOffset - (farHeight / STAIRCASE.totalHeight) * (Math.PI * 2 * STAIRCASE.turns);

  // 終點需要修正多少角度，取最短路徑（避免繞一大圈跳動），3D 數學中將角度約束在 $[-\pi, +\pi]$（即 $-180^\circ \sim +180^\circ$）的標準作法。它能自動判斷「順時針還是逆時針比較近」，永遠強制選擇「最短轉向路徑」
...
async function saveFilterUsage(device) {
  if (!FILTER_PERSIST_ENABLED) return; // 如果全域設定檔中的「資料庫儲存功能」是關閉的（false），就直接 return 結束，不發送網路請求
  const st = filterState[device];
  const deltaToFlush = st.pendingDelta; // 取出本機目前「還沒寫入資料庫的暫存秒數」
...
class WaterFlow {
  ...
  _build() {
    ...
    //  把填好的數字陣列轉為 BufferAttribute，並綁定到 BufferGeometry 上
    ...
    const mat = new THREE.PointsMaterial({
      ...
      transparent: true,
      opacity: 0.85,
      blending: THREE.AdditiveBlending, // 加法混合（Additive Blending, 當水滴粒子互相重疊，或是重疊在其他 3D 物件上時，顏色會直接加起來變亮
      depthWrite: false,
  ...
  _resetVelocity(i) {
    if (this.type === 'faucet') {
      this.velocities[i] = new THREE.Vector3(
        (Math.random() - 0.5) * 0.015,     // X 軸（左右微幅抖動）
        -(0.025 + Math.random() * 0.015),   // Y 軸（向下垂直噴射）
        (Math.random() - 0.5) * 0.015      // Z 軸（前後微幅抖動
      );
    } else {
      const angle = Math.random() * Math.PI * 2;   // 隨機角度 0 ~ 360 度
      const radius = 0.010 + Math.random() * 0.03;  // 隨機擴散半徑
      this.velocities[i] = new THREE.Vector3(
        Math.cos(angle) * radius,       // X 軸（極座標轉廣角擴散）
        -(0.02 + Math.random() * 0.02), // Y 軸（向下噴射速度）
        Math.sin(angle) * radius        // Z 軸（極座標轉廣角擴散）
      );
    }
  }
...
class DrainFlow {
  ...
  _initParticle(i, randomStart = false) {
    ...
    this.particles[i] = {
      r,
      angle,
      baseSpeed: 0.6 + Math.random() * 0.8,   // 低基礎速度
  ...
  setActive(isActive) {
    ...
    this.points.visible = isActive;
    if (isActive) {
      this.fadeOpacity = 0; // 將起始透明度歸零（完全透明）
      this.fadeElapsed = 0; // 將淡入計時器重置為 0 秒
    }
  }
  ...
  update(delta) {
    ...
    // 計算正弦波閃爍頻率, 水流在噴射時，透明度會以自然的頻率進行微幅的波浪式閃爍，模擬真實水流折射光線時的折光透亮感（不會死板地維持固定透明度）
    const flicker = 0.78 + Math.sin(performance.now() * 0.004) * 0.15;
    this.points.material.opacity = flicker * this.fadeOpacity;
  }

  dispose() {
    this.scene.remove(this.points);   // 1. 從 3D 場景樹中移除物件
    this.points.geometry.dispose();  // 2. 清除顯存中的頂點 Buffer 數據
    this.points.material.dispose();  // 3. 清除顯存中的 Shader 著色器與材質數據
  }
}
// ─────────────────────────────────────────
// 三、場景、渲染器、後處理
...
// ⚡ 新增：AudioListener 是 Web Audio 的「耳朵」，掛在攝影機上代表
// 音量/左右聲道會依攝影機（也就是玩家）的位置/朝向計算，
// 跟 PositionalAudio（掛在音源物件上，代表「嘴巴」）搭配使用。
...
renderer.debug.checkShaderErrors = false;//  關鍵優化：關閉 Shader 編譯時的錯誤檢查（避免 CPU/GPU 同步阻塞）, 關閉顯示卡在編譯 Shader（著色器）時的『除錯檢查』，避免 CPU 硬生生在那邊等待 GPU 回傳結果，解決 3D 畫面開場或切換材質時瞬間『卡頓（Jank/Stutter）』的問題

renderer.setSize(window.innerWidth, window.innerHeight);
renderer.setPixelRatio(Math.min(window.devicePixelRatio, 1.5)); // 根據使用者的螢幕像素密度來渲染畫面，但像素倍率最高鎖定在 1.5 倍就好，絕對不要盲目跟隨 Retina 螢幕開到 2 倍甚至 3 倍
...
document.body.appendChild(renderer.domElement); // 把 Three.js 在幕後建立好的 <canvas> 繪圖畫布，正式插入到 HTML 網頁的 <body> 標籤中，讓顯示卡算出來的 3D 畫面能夠真正呈現在使用者的螢幕上。domElement 就是那張畫布。顯示卡（GPU）渲染出的所有 3D 畫面（包括房間、管路、水流粒子），最後都會被畫在這張 <canvas> 上

const composer = new EffectComposer(renderer); // 在幕後建立緩衝區（Frame Buffer），將畫面一層層疊加加工後才輸出
composer.addPass(new RenderPass(scene, camera)); // RenderPass 負責先把標準的 3D 場景（包含幾何體、牆壁、水滴粒子）繪製出來，作為後續濾鏡加工的原始底圖
composer.addPass(new UnrealBloomPass(
  new THREE.Vector2(window.innerWidth * 0.2, window.innerHeight * 0.2), // 1. 低解析度渲染 (優化關鍵)
  0.2,                                                                    // 2. 輝光強度 (Strength)
  0.5,                                                                    // 3. 輝光半徑 (Radius)
  0.85                                                                    // 4. 發光門檻 (Threshold)
)); // 模擬虛幻引擎（Unreal Engine）等級的渲染光暈
...
RectAreaLightUniformsLib.init(); // 1.載入 LTC 預計算紋理數據：它會在記憶體中初始化兩張數據查找表（LTC Lookup Tables）。2.注入 GLSL 着色器代碼：將區域光的複雜光照數學公式，動態掛載到 Three.js 的內置 Shader 系統中。
...
function getDeviceType(name) {
  const cfg = PIPE_CONFIG[name]; // 1. 直接拿 name 當 Key 去查配置檔
  return cfg ? cfg.type : null;  // 2. 查得到就回傳 type，查不到回傳 null 防呆
}

// 擬真圓錐體體積光（Volumetric Spotlight / Light Cone）」 的客製化 Shader, 用一個圓錐體網格（Cone Geometry）搭配專屬著色器（GLSL Shader），打造出像舞台燈光、手電筒、或探照燈穿透空氣時那種「肉眼可見的半透明光束/光霧」
function createConeVolumetricLight(color) {
  const h = 3.2;
  const geo = new THREE.ConeGeometry(0.55, h, 32, 1, true); // 用最少、最輕量的頂點（32邊形、1層高度、無底板），形塑出一個滑順空心的「發光燈罩/光錐」
  ...
  mesh.position.y = -h / 2; // Three.js 圓錐體的原點預設在幾何中心。向下偏移 h/2 後，讓頂點（尖端出光處）對齊原點 $(0, 0, 0)
  return mesh;
}
...
const MAX_SLIDE_ITERATIONS = 3; // 防止玩家在牆角、夾角、死角處穿牆破圖, 撞到牆壁時不要只轉彎一次，要在同 0.016 秒（同一幀）裡面『一直微調方向』直到徹底沒撞到牆為止，最多允許嘗試 3 次。
...
const NEARBY_COLLISION_RADIUS = 8; // 半徑 8 公尺的篩選
...
function getNearbyCollidables(sourceObjects, sourceSpheres) {
  _nearbyFilterResult.length = 0; // 清空舊資料（零記憶體垃圾）
...
function handleMovementAndCollision(moveVelocity) {
  ...
  // 如果现在是透視模式，就走一套简化版的规则：只檢查會不會撞到房子最外層的幾道牆，撞到就贴着滑；没撞到就直接走過去，其他複雜的规則（一般模式那堆牆、樓梯的特殊判斷）这时候统统不用管，直接跳過
  if (isNoclipMode) {
  ...
  // 第 1 次修正：撞到牆壁 A ──► 速度被修正為「沿著牆壁 A 滑動」 (方向指向牆壁 B)
  // 第 2 次修正：撞到牆壁 B ──► 速度再次被修正為「沿著牆壁 B 滑動」 (方向遠離牆角)
  // 第 3 次檢查：兩邊都不撞了 ──► moveVelocity 沒有變化，觸發 break 跳出！
...
function updateStaircase(delta) {
  ...
  const canLeaveTop = stairCommitted ? outwardAmount > 0 : inputDir > 0; // canLeaveTop是布林值
  const canLeaveBottom = stairCommitted ? outwardAmount > 0 : inputDir < 0;

  if (stairHeight >= STAIRCASE.totalHeight && canLeaveTop) {
    stairHeight = STAIRCASE.totalHeight; // 1. 高度封頂，防止爆高
    isOnStairRail = false;              // 2. 關閉樓梯軌道鎖定（解鎖狀態）
    return;                             // 3. 跳出函式，回到自由移動邏輯
  }
...
// ─────────────────────────────────────────
// ✅ 只宣告一次 manager
const manager = new THREE.LoadingManager(); // 製作「載入中...（Loading...）」進度條
...
Object.entries(AUDIO_FILES).forEach(([type, url]) => {
  audioLoader.load(url, (buffer) => {
    audioBuffers[type] = buffer;
  }, undefined, (err) => {
    console.warn(`[音效] 載入 ${type} 音效失敗`, err);
  });
});

// ⚡ 幫指定裝置建立一個掛在出水口位置上的空間音效音源。
// 一定要等 outletObjects 填好、且 audioBuffers 對應類型已經載入完成才能呼叫，
// 所以放在 loader.load(CONFIG.MODELS.BUILDING, ...) callback 的最後面呼叫（見下方步驟3）。
function createDeviceWaterSound(deviceName) {
  const cfg = PIPE_CONFIG[deviceName];
  if (!cfg) return;

  const buffer = audioBuffers[cfg.type]; // 'faucet' 或 'shower'
  if (!buffer) {
    console.warn(`[音效] ${cfg.type} 音效尚未載入完成，${deviceName} 無法建立音源`);
    return;
  }

  const anchor = outletObjects[cfg.outletKey];
  if (!anchor) return;

  const sound = new THREE.PositionalAudio(audioListener);
  sound.setBuffer(buffer);
  sound.setLoop(true);
  sound.setRefDistance(1.5);  // 音量開始衰減的距離（公尺），可依場景大小調整
  sound.setMaxDistance(12);   // 超過這個距離幾乎聽不到
  sound.setVolume(cfg.type === 'shower' ? 0.6 : 0.4);
  anchor.add(sound); // 掛在出水口上，位置會自動跟著裝置走
...
function computeLeafEntry() {
  ...
  const distance = Math.max(w, h) * 0.42 + 140; // 「视窗较长的那一边」的长度，缩放成 42% 的距离，确保不管视窗多小，都还有最低限度的额外缓冲距离，让叶子一定是从画面看不到的地方开始飛進來

  const angleDeg = -150 + Math.random() * 120;
  const rad = angleDeg * Math.PI / 180; // JavaScript 的三角函数（Math.sin、Math.cos）,吃的参数单位是「弧度（radian）」，不是我们日常习惯讲的「度（degree）」
  ...
  const mag = Math.hypot(p0x, p0y) || 1; // // 計算向量 (p0x, p0y) 到原點 (0,0) 的距離，如果 Math.hypot(p0x, p0y) 算出來是 0（在 JS 中 0 會被當作 false），|| 就會自動發揮防呆效果，退而求其次選擇右邊的 1
  const perpX = -p0y / mag;
  const perpY = p0x / mag;
...
function animateLeafFlight(points, duration) {
  ...
  let rippleTriggered = false; // ⚡ 新增：這次飛行水波紋是否已提早觸發過，如果沒有這個 rippleTriggered 旗標，系統會在水滴經過水面的那幾十個影格內重複生成幾十個水波紋，導致特效重疊暴走或 GPU 記憶體爆掉
...
const tapPrompt = document.createElement('div'); // 在記憶體中動態建立一個全新的 HTML <div> 標籤元素
...
function showTapPrompt() {
  ...
  leafEntryDelayTimeoutId = setTimeout(() => {
    ...
    leafStage.style.transform = `translate(${points.p0x.toFixed(1)}px, ${points.p0y.toFixed(1)}px)`; // 1. 先把葉子貼到起點位置
    void leafStage.offsetWidth; // 2. 強迫瀏覽器「立刻渲染起點」，確保葉子真的已經在起點上了
    animateLeafFlight(points, LEAF_FLIGHT_DURATION); // 3. （接下來的程式碼）才開始播放飄飛動畫，動畫才會順暢地從起點開始動
    leafEntryDelayTimeoutId = null;
  }, 500); // 當計時器裡面的任務執行完畢後，把原本存的計時器 ID 清空（設為 null）
}
...
function hideTapPrompt() {
  ...
  setTimeout(() => {
    ...
    tapPromptRipple.classList.remove('rippling'); // 把漣漪/水波紋效果（Ripple effect）的 Class 拔掉，停止點擊動畫
    if (leafFlightRafId !== null) {
      cancelAnimationFrame(leafFlightRafId);
      leafFlightRafId = null;
    }
    stopLeafSway(); // 呼叫 stopLeafSway()，用來停止葉子飄動/搖晃的離散動畫或狀態
    leafStage.style.transform = 'translate(0px, 0px)'; // 把葉子（leafStage）的位置重置回初始原點 (0px, 0px)
    tapPromptWelcome.style.opacity = '0'; // 將歡迎訊息（tapPromptWelcome）的透明度也歸零
  }, 400); // 400 等淡出完成後，才真正把元素從畫面上隱藏並重置狀態
}
...
rgbeLoader.load(CONFIG.MODELS.HDRI, (hdr) => {
  hdr.mapping = THREE.EquirectangularReflectionMapping;// 1. 設定貼圖解讀方式（把扁平圖變球體）
  scene.environment = hdr; // 2. 當作物理光源與反射（讓 3D 模型吸收這張圖的光線與倒影）
  scene.background = hdr; // 3. 當作視窗背景（讓眼睛看得到這張圖）
});
...
loader.load(CONFIG.MODELS.BUILDING, (gltf) => {
  ...
  gltf.scene.traverse((obj) => {
    ...
    // ⚡ 新增：名稱含 "sliding_down_door" 的門視為「往下滑動」的滑門
    const isDownSlidingDoor = name.includes('sliding_down_door');
    ...
    if (isTurnUpDoor) {
      ...
      const doorIndicatorMeta = computeDoorIndicatorMeta(obj);
      ...
      doorAnimations[name] = {
        ...
        direction: 0,
        type: 'turn_up',
        turnUpSign: TURN_UP_SIGN_OVERRIDE[name] ?? -1,
        turnUpAxis: TURN_UP_AXIS_OVERRIDE[name] ?? 'z',
        baseRotationX: obj.rotation.x,
        baseRotationY: obj.rotation.y,
        baseRotationZ: obj.rotation.z,
        // ⚡ 指示器用：門的本地中心點
        frontAxis: DOOR_FRONT_NORMAL_AXIS_OVERRIDE[name] ?? doorIndicatorMeta.frontAxis,
        indicatorLocalX: doorIndicatorMeta.doorCenterX,
        indicatorLocalY: doorIndicatorMeta.doorCenterY,
      };
    ...
    // 往上/往下滑動的門固定用 Y 軸，不套用原本 X/Z 自動判斷邏輯
    const slideAxis = (isUpSlidingDoor || isDownSlidingDoor)
      ? 'y'
      : (SLIDE_AXIS_OVERRIDE[name] ?? (size.x >= size.z ? 'x' : 'z'));
    const slideDistance = (isUpSlidingDoor || isDownSlidingDoor)
      ? Math.max(0, size.y - DOOR_OPEN_MARGIN)
      : Math.max(0, (slideAxis === 'z' ? size.z : size.x) - DOOR_OPEN_MARGIN);
    // 往上滑動預設方向是 +Y（往上開），往下滑動預設方向是 -Y（往下開），
    // 一般滑門預設是 -1；都可用 SLIDE_SIGN_OVERRIDE 覆寫
  ...
  gltf.scene.traverse((mesh) => {
    ...
    if (name.includes('door_') && !doorAnimations[name]) {
      ...
      // ⚡ 新增：用門片自己的「本地」geometry 包圍盒（不受父層旋轉影響），
      // 算出門片寬度、垂直中心、以及門把該偏向哪一側（離原點/門軸較遠的那一邊）。
      // 供 attachDoorIndicatorsToScene() 換算門把指示器的位置用。
      const doorIndicatorMeta = computeDoorIndicatorMeta(mesh);
      ...
      doorAnimations[name] = {
        ...
        direction: 0,
        type: 'swing',
        swingSign: SWING_SIGN_OVERRIDE[name] ?? -1,
        baseRotationY: mesh.rotation.y,
        // ⚡ 指示器用：門的本地中心點（不再用門把偏移邏輯）
        frontAxis: DOOR_FRONT_NORMAL_AXIS_OVERRIDE[name] ?? doorIndicatorMeta.frontAxis,
        indicatorLocalX: doorIndicatorMeta.doorCenterX,
        indicatorLocalY: doorIndicatorMeta.doorCenterY,
      };
  ...
  // ✅ 建立水流音效（如果音效檔案還沒載入完成，這裡會先跳過，
  // 等音效真正載入好之後不會自動補建立——如果你的音效檔案較大、
  // 擔心跟建築模型載入時機沒對齊，可以改用下方「保險寫法」）
  interactiveDevices.forEach(mesh => {
    createDeviceWaterSound(mesh.name.toLowerCase());
  });
...
const DOOR_INDICATOR_HEIGHT_OFFSET = 1.0;   // 掛在門上的垂直偏移，可依實際模型調整
const DOOR_INDICATOR_DIR_SAMPLE_DIST = 0.6; // 世界座標中沿開門方向取樣多遠，用來算螢幕角度
...
// 一定要等 GLTF 載入完成、doorAnimations 都填好之後才能呼叫，
// 建議直接接在 attachDoorHintsToScene(); 那一行後面一起呼叫。
function attachDoorIndicatorsToScene() {
  Object.keys(doorAnimations).forEach(name => {
    if (!doorIndicatorInstances[name]) createDoorIndicator(name);
    const inst = doorIndicatorInstances[name];
    if (inst.cssObject) return;

    const anim = doorAnimations[name];
    if (!anim || !anim.mesh) return;

    const cssObject = new CSS2DObject(inst.wrapper);
    // ⚡ 不分門型，統一用門的本地中心點（水平＋垂直都置中）
    const x = anim.indicatorLocalX ?? 0;
    const y = anim.indicatorLocalY ?? DOOR_INDICATOR_HEIGHT_OFFSET;
    cssObject.position.set(x, y, 0);

    anim.mesh.add(cssObject);
    inst.cssObject = cssObject;
  });
}

const doorIndicatorStyleTag = document.createElement('style');
doorIndicatorStyleTag.textContent = `
.door-indicator-root {
  position: absolute;
  left: 50%;
  top: 0;
...
Object.assign(climatePanel.style, {
  ...
  display: 'flex',
  flexDirection: 'column', // 垂直排列：溫度→濕度→執行
  gap: '8px',
  zIndex: '250',
  pointerEvents: 'none',
  opacity: '0',
...
const climateTemperatureBox = makeClimateBox();
const climateHumidityBox = makeClimateBox();
const climateActionBox = makeClimateBox();
climatePanel.appendChild(climateTemperatureBox);
climatePanel.appendChild(climateHumidityBox);
climatePanel.appendChild(climateActionBox);
document.body.appendChild(climatePanel);

// ── 依溫濕度決定執行框文字 ──
function getClimateActionText(temp, humidity) {
  const actions = [];
  if (temp > 28) actions.push('已開啟冷氣');
  if (humidity > 60) actions.push('已開啟除濕');
  return actions.length > 0 ? actions.join('、') : '環境舒適';
}
...
function showClimateHintBoxes() {
  if (currentOutdoorTemp === null || currentOutdoorHumidity === null) {
    // ⚡ 氣象資料還沒抓到/抓取失敗時，也要讓玩家知道發生了什麼事，
    // 而不是悄悄什麼都不顯示。
    climateTemperatureBox.textContent = '🌡️ 氣象資料讀取中...';
    climateHumidityBox.textContent = '💧 氣象資料讀取中...';
    climateActionBox.textContent = '⚙️ 請稍候';
  } else {
    climateTemperatureBox.textContent = `🌡️ 室外溫度 ${Math.round(currentOutdoorTemp)}°C`;
    climateHumidityBox.textContent = `💧 室外濕度 ${Math.round(currentOutdoorHumidity)}%`;
    climateActionBox.textContent = `⚙️ ${getClimateActionText(currentOutdoorTemp, currentOutdoorHumidity)}`;
  }

  climatePanel.style.opacity = '1';
  climatePanel.style.transform = 'translate(0, -50%)';

  clearTimeout(climateHideTimer);
  climateHideTimer = setTimeout(() => {
    climatePanel.style.opacity = '0';
    climatePanel.style.transform = 'translate(80px, -50%)';
  }, 3000); // 維持3秒後自動收回
}
...
const _doorIndicatorP1 = new THREE.Vector3();
const _doorIndicatorInvMatrix = new THREE.Matrix4();
const _doorIndicatorLocalCamera = new THREE.Vector3();
const _doorIndicatorLocalCenter = new THREE.Vector3();
const _doorIndicatorLocalSample = new THREE.Vector3();
const _doorIndicatorWorldSample = new THREE.Vector3();

function updateDoorIndicators(delta) {
  // ⚡ 總開關關閉時：把目前還顯示中的指示器全部隱藏，並直接跳過所有計算，
  // 不會浪費效能繼續算距離/角度。
  if (!window.SHOW_DOOR_INDICATORS) {
    Object.keys(doorIndicatorInstances).forEach(name => {
      const inst = doorIndicatorInstances[name];
      if (inst && inst.isVisible) {
        inst.isVisible = false;
        inst.root.classList.remove('visible');
      }
    });
    return;
  }
  ...
  Object.keys(doorAnimations).forEach(name => {
    const inst = doorIndicatorInstances[name];
    const anim = doorAnimations[name];
    if (!inst || !inst.cssObject || !anim) return;
    ...
    inst.cssObject.getWorldPosition(_doorIndicatorAnchor);
    _doorIndicatorP0.copy(_doorIndicatorAnchor).project(camera);

    const dist = camera.position.distanceTo(_doorIndicatorAnchor);
    const inRange = dist <= DOOR_INDICATOR_RADIUS && _doorIndicatorP0.z < 1;

    if (inRange !== inst.isVisible) {
      inst.isVisible = inRange;
      inst.root.classList.toggle('visible', inRange);
      if (inRange) inst.elapsed = DOOR_INDICATOR_INTERVAL;
    }
    if (!inRange) return;

    // ⚡ 用完整 matrixWorld（含旋轉＋縮放，能正確處理鏡射門）把攝影機位置
    // 轉換回門的本地座標系，比較「厚度軸」上的分量，判斷camera站在哪一側——
    // 哪一側比較近就自動指向哪一側，不需要每扇門手動設定正負號。
    anim.mesh.updateMatrixWorld();
    _doorIndicatorInvMatrix.copy(anim.mesh.matrixWorld).invert();
    _doorIndicatorLocalCamera.copy(camera.position).applyMatrix4(_doorIndicatorInvMatrix);

    _doorIndicatorLocalCenter.set(
      anim.indicatorLocalX ?? 0,
      anim.indicatorLocalY ?? 0,
      0
    );

    // ⚡ 三角形指向「開門方向」，不是指向使用者站的那一側——
    // 用回 getDoorOpenLocalDirection() 算出的本地開門方向向量，
    // 從門的中心點往那個方向取樣一小段距離，再投影到螢幕算角度。
    const openDirLocal = getDoorOpenLocalDirection(anim);
    _doorIndicatorLocalSample.copy(_doorIndicatorLocalCenter)
      .addScaledVector(openDirLocal, DOOR_INDICATOR_DIR_SAMPLE_DIST);
    _doorIndicatorWorldSample.copy(_doorIndicatorLocalSample).applyMatrix4(anim.mesh.matrixWorld);
    _doorIndicatorP1.copy(_doorIndicatorWorldSample).project(camera);

    const sx0 = (_doorIndicatorP0.x + 1) / 2 * window.innerWidth;
    const sy0 = (1 - _doorIndicatorP0.y) / 2 * window.innerHeight;
    const sx1 = (_doorIndicatorP1.x + 1) / 2 * window.innerWidth;
    const sy1 = (1 - _doorIndicatorP1.y) / 2 * window.innerHeight;

    const dx = sx1 - sx0;
    const dy = sy1 - sy0;
    const angleDeg = (Math.abs(dx) > 1e-6 || Math.abs(dy) > 1e-6)
      ? Math.atan2(dy, dx) * 180 / Math.PI
      : 0;

    inst.group.style.transform = `rotate(${angleDeg.toFixed(1)}deg)`;
...
renderer.domElement.addEventListener('click', () => {
  ...
  // ⚡ 水流音效：開水播放（如果還沒開始播），關水暫停（不用重建，暫停/播放成本極低）
  const sound = deviceWaterSounds[targetName];
  if (sound) {
    if (isNowActive && !sound.isPlaying) sound.play();
    else if (!isNowActive && sound.isPlaying) sound.stop();
  }

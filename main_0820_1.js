...
function finishLoading() {
  ...
  setTimeout(() => {
    ...
    // ⚡ 再多等一小段時間，讓場景初始化的尾巴、以及暖機渲染可能延遲發生的
    // shader 編譯成本，都有機會在葉子動畫真正啟動前徹底完成
    setTimeout(showTapPrompt, 150);
  }, 600);
}
...
Object.assign(tapPromptWelcome.style, {
  ...
  opacity: '0',
  transition: 'opacity 1.0s ease', // ⚡ 1.6s → 1.1s，比原本 0.8s 稍長，兼顧緩衝效果又不會太晚才顯現
});
...
function showTapPrompt() {
  ...
  tapPromptLeaf.style.opacity = '0'; // ⚡ 初始透明，交給 animateLeafFlight() 裡的 rAF 控制淡入

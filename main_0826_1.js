...
// ── 圓環四色分區設定：0~100 對應「已使用百分比」，數字越大代表濾心越接近該換 ──
// ⚡ 原本是5段（藍/綠/黃/橘/紅），改成4段：0~50合併成單一藍色，
// 拿掉中間的綠色分段。理由：圓環改成「單色提醒」邏輯後（見下方 updateFilterUI），
// 分區數量越少，使用者一眼看到的顏色語意越單純（安全/注意/警戒/危險 4個等級）。
const FILTER_RING_ZONES = [
  { min: 0, max: 50, color: '#60a5fa', dim: 'rgba(59,130,246,0.25)' },   // 藍：安全
...
function updateFilterUI(device) {
  ...
  // 只需要同步顏色，讓光暈跟著目前所在區段（藍/黃/橘/紅）換色即可。

...
loader.load(CONFIG.MODELS.BUILDING, (gltf) => {
  ...
  gltf.scene.traverse((mesh) => {
    ...
    if (name !== 'floor_pool_water' &&
        (name.includes('wall') || name.includes('floor') || name.includes('door') || name.includes('entrance') || name.includes('stairs'))) {
...
function animate() {
  ...
  for (const key in doorAnimations) {
    ...
    if (anim.type === 'sliding') {
      ...
      // 上掀式門：繞 X 軸旋轉，在「原本角度」基礎上疊加旋轉到 60 度
      anim.mesh.rotation.z = (anim.baseRotationX ?? 0) + (anim.turnUpSign ?? -1) * eased * TURN_UP_OPEN_ANGLE;
    } else {

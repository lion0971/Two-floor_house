...
/* 葉片：從大縮小到最小，中途微微搖晃（左右擺動＋小角度旋轉），
   0%~4% 由透明淡入、96%~100% 淡出到透明，讓 infinite 循環時
   頭尾銜接處不會出現「瞬間跳回原點」的閃爍感 */
@keyframes tapLeafFall {
  0%   { transform: scale(1.6) rotate(-6deg)  translateX(0);    opacity: 0; }
  4%   { transform: scale(1.55) rotate(-4deg) translateX(2px);  opacity: 1; }
  22%  { transform: scale(1.25) rotate(9deg)  translateX(8px);  opacity: 1; }
  40%  { transform: scale(0.95) rotate(-11deg) translateX(-9px); opacity: 1; }
  58%  { transform: scale(0.68) rotate(7deg)  translateX(6px);  opacity: 1; }
  76%  { transform: scale(0.46) rotate(-5deg) translateX(-3px); opacity: 1; }
  90%  { transform: scale(0.36) rotate(2deg)  translateX(0);    opacity: 1; }
  96%  { transform: scale(0.34) rotate(0deg)  translateX(0);    opacity: 0; }
  100% { transform: scale(0.34) rotate(0deg)  translateX(0);    opacity: 0; }
}

/* 水波紋：跟葉片動畫「同樣總時長、同步循環」，只在葉片快落到最小的
   後段（76%~100%）才展開淡出，時間點對齊葉片縮到最小的那一刻 */
@keyframes tapRippleExpand {
  0%, 74%  { opacity: 0;   transform: translate(-50%, -50%) scale(0.25); }
  80%      { opacity: 0.55; transform: translate(-50%, -50%) scale(0.6); }
...
.tap-prompt-leaf {
  ...
  animation: tapLeafFall 2.6s ease-in-out infinite;
...
.tap-prompt-ripple {
  ...
  animation: tapRippleExpand 2.6s ease-in-out infinite;
}
`;
...
// ⚡ 改用 emoji 文字，直接沿用 index.html 裡 <div class="leaf-icon">🌿</div> 同一顆葉子，
// 不需要額外的圖片檔案，也少一次網路請求。

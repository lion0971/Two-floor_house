...
import { getDatabase, ref, onValue, get, set, remove } from "https://www.gstatic.com/firebasejs/10.12.4/firebase-database.js";
...
FILTER_DEVICES.forEach(name => {
  filterState[name] = {
    ...
    saveElapsed: 0,
    alerted: false,
    ready: false, // 尚未從 Firebase 讀到舊資料前，先不要累加，避免用 0 覆蓋掉
  };
});
...
async function loadFilterUsage() {
  if (!FILTER_PERSIST_ENABLED) {
    // 沒開濾心持久化功能時可以不連 Firebase，濾心壽命從本機 0 開始算即可
    FILTER_DEVICES.forEach(name => { filterState[name].ready = true; });
    return;
  }
  await authReadyPromise;
  for (const name of FILTER_DEVICES) {
    try {
      const snap = await get(ref(db, `filters/${name}`));
      const data = snap.exists() ? snap.val() : null;
      filterState[name].usedSeconds = data?.usedSeconds ?? 0;
      filterState[name].alerted = data?.alerted ?? false;
    } catch (err) {
      console.warn(`[Filter] 讀取 ${name} 濾心用量失敗`, err);
    } finally {
      filterState[name].ready = true;
      updateFilterUI(name); // 讀到舊資料後，先把圓環畫成正確的初始狀態
    }
  }
}
loadFilterUsage();
...
async function saveFilterUsage(device) {
  ...
  try {
    await set(ref(db, `filters/${device}`), {
      usedSeconds: filterState[device].usedSeconds,
      alerted: filterState[device].alerted,
      updatedAt: Date.now(),
    });
  } catch (err) {
...
function resetFilterUsage(device) {
  filterState[device].usedSeconds = 0;
  filterState[device].alerted = false;
...
// ─────────────────────────────────────────
// 濾心壽命 UI（畫面右上方，約右三分之一、上三分之一位置）
// ─────────────────────────────────────────
// ⚡ pointerEvents: 'none'：純顯示用途，不擋滑鼠/觸控操作，成本上也不需要接收事件
const filterUIContainer = document.createElement('div');
Object.assign(filterUIContainer.style, {
  position: 'fixed',
  top: '33%',      // 畫面上三分之一
  right: '33%',    // 畫面右三分之一
  display: 'flex',
...
document.body.appendChild(filterUIContainer);

const filterUICards = {}; // { [device]: { root, fg, percentText, hourLine } }
...
function createFilterCard(device) {
  ...
  Object.assign(root.style, {
    display: 'none', // 該裝置沒出水時隱藏，出水才顯示，避免畫面一開始就塞滿卡片
    alignItems: 'center',
    gap: '10px',
    ...
    minWidth: '210px',
    // ⚡ 展開＋回彈動畫用的初始狀態與過渡設定：
    // transformOrigin 改成「左側中間」，讓卡片放大時是「左邊固定不動，往右展開」，
    // 避免展開方向朝畫面左邊（水龍頭視角所在的方向）蓋過去。
    // transform 初始值把卡片放在「比最終位置再低一點」的地方（translateY 正值），
    // 展開時會往上彈進最終位置，配合 cubic-bezier(0.34, 1.8, 0.64, 1) 這個
    // 「easeOutBack」曲線，往上衝過一點再彈回來，做出向上彈跳的感覺。
    // opacity 的淡入時間拉長到 0.6 秒（比原本慢），讓「從透明到完全顯現」
    // 這段過程感覺更柔和、不會一下子就整個跳出來。
    transformOrigin: 'center left',
    opacity: '0',
    transform: 'scale(0.4) translateY(18px)', // 跟 FILTER_CARD_HIDDEN_TRANSFORM 常數保持一致（此處尚未定義該常數，故直接寫值）
    transition: 'transform 0.5s cubic-bezier(0.34, 1.8, 0.64, 1), opacity 0.6s ease',
  });
  ...
  bg.setAttribute('cx', '30'); bg.setAttribute('cy', '30'); bg.setAttribute('r', String(RING_RADIUS));
  ...
  fg.setAttribute('cx', '30'); fg.setAttribute('cy', '30'); fg.setAttribute('r', String(RING_RADIUS));
  fg.setAttribute('fill', 'none');
  fg.setAttribute('stroke', '#00e0ff');
  ...
  fg.setAttribute('transform', 'rotate(-90 30 30)'); // 指針從正上方12點鐘方向開始畫
  svg.appendChild(fg);

  const percentText = document.createElementNS('http://www.w3.org/2000/svg', 'text');
  percentText.setAttribute('x', '30'); percentText.setAttribute('y', '34');
  ...
  nameLine.style.opacity = '0.75';
  nameLine.style.fontSize = '11px';
  nameLine.textContent = DEVICE_LABEL[device] ?? device;
  const hourLine = document.createElement('div');
  hourLine.style.fontWeight = 'bold';
  infoBox.appendChild(nameLine);
  infoBox.appendChild(hourLine);

  root.appendChild(svg);
  root.appendChild(infoBox);
  filterUIContainer.appendChild(root);

  filterUICards[device] = { root, fg, percentText, hourLine };
}
...
function updateFilterUI(device) {
  ...
  const remainRatio = Math.max(0, 1 - st.usedSeconds / FILTER_LIFETIME_SECONDS);
  const percent = Math.round(remainRatio * 100);

  card.fg.setAttribute('stroke-dashoffset', String(RING_CIRC * (1 - remainRatio)));
  card.fg.setAttribute('stroke', remainRatio <= FILTER_LOW_RATIO ? '#ff5533' : '#00e0ff');
  card.percentText.textContent = percent + '%';
  card.hourLine.textContent = `已用 ${formatUsedDuration(st.usedSeconds)}（總容量 ${FILTER_LIFETIME_HOURS} 小時）`;
}

// ⚡ 展開＋回彈動畫的顯示間隔（要跟卡片 transition 的秒數對齊）；
// 關閉時使用者希望「俐落、立刻消失」，所以不需要對應的隱藏動畫秒數。
const FILTER_CARD_SHOW_MS = 500;
const FILTER_CARD_HIDDEN_TRANSFORM = 'scale(0.4) translateY(18px)'; // 跟 createFilterCard 的初始值一致

function showFilterCard(card) {
  // 如果剛好正在播放「收起」的動畫，先取消，避免兩個動畫互相搶
  if (card.hideTimer) {
    clearTimeout(card.hideTimer);
    card.hideTimer = null;
  }
  card.root.style.display = 'flex';
  // ⚡ 關鍵技巧：從 display:none 換成 display:flex 的那一瞬間，
  // 卡片目前的 opacity/transform 還停留在「縮小透明」的初始值，
  // 但瀏覽器還沒有機會把這個初始狀態「畫出來」。如果馬上接著就把
  // opacity/transform 改成展開後的最終值，瀏覽器會直接跳到最終狀態，
  // 完全沒有 transition 過渡效果（等於還是「瞬間出現」）。
  // 用兩層 requestAnimationFrame，確保瀏覽器先完成一次畫面繪製
  // （讓「縮小透明」這個起始狀態真正生效），下一輪才套用最終狀態，
  // 這樣 CSS transition 才會確實從頭播放展開＋回彈的動畫。
  requestAnimationFrame(() => {
    requestAnimationFrame(() => {
      card.root.style.opacity = '1';
      card.root.style.transform = 'scale(1) translateY(0)';
    });
  });
}
...
function hideFilterCard(card) {
  ...
  // ⚡ 關閉水龍頭時要「俐落、立刻消失」，不要有縮小/淡出動畫造成的停留感，
  // 所以這裡不套用 transition，直接把 display 設回 none。
  // 同時把 opacity/transform 重置回初始值，這樣下次這個裝置再次出水時，
  // showFilterCard() 才會重新從「縮小透明」開始，正常播放一次完整的展開＋回彈動畫，
  // 而不是從上次殘留的狀態直接跳出來。
  card.root.style.display = 'none';
  card.root.style.opacity = '0';
  card.root.style.transform = FILTER_CARD_HIDDEN_TRANSFORM;
}
...
function updateFilterUIVisibility() {
  FILTER_DEVICES.forEach(device => {
    const card = filterUICards[device];
    if (!card) return;
    if (filterState[device].isActive) {
      showFilterCard(card);
    } else {
      hideFilterCard(card);
    }
...
function animate() {
  ...
  for (const device of FILTER_DEVICES) {
    ...
    if (st.usedSeconds < FILTER_LIFETIME_SECONDS) {
      st.usedSeconds = Math.min(st.usedSeconds + delta, FILTER_LIFETIME_SECONDS);
    }
    st.uiElapsed += delta;

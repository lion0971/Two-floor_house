...
function updateStaircase(delta) {
  ...
  if (inputZ !== 0 || inputX !== 0) {
    ...
    // 內積：只要玩家有明顯往樓梯中心以外走的意圖（不管跟切線角度對不對得
    // 上），就視為「想離開」，下面放行條件會用 OR 把這個也算進去——
  ...
  // ⚡ 徑向往外走的意圖超過這個門檻，視為「明顯想離開樓梯」，
  // 不用刻意對準切線角度也能放行（容許範圍，跟入口角度容差同樣的精神）。
  const STAIR_EXIT_OUTWARD_THRESHOLD = 0.35;

  if (stairHeight >= STAIRCASE.totalHeight && (inputDir > 0 || outwardAmount > STAIR_EXIT_OUTWARD_THRESHOLD)) {
    stairHeight = STAIRCASE.totalHeight;
    isOnStairRail = false;
    return;
  }
  if (stairHeight <= 0 && (inputDir < 0 || outwardAmount > STAIR_EXIT_OUTWARD_THRESHOLD)) {
    stairHeight = 0;
    isOnStairRail = false;
    return;
  }

...
function updateFilterUI(device) {
  ...
  const usedHours = (st.usedSeconds / 3600).toFixed(1);
  ...
  card.hourLine.textContent = `已用 ${usedHours} / ${FILTER_LIFETIME_HOURS} 小時`;
}
...
function updateFilterUIVisibility() {
  FILTER_DEVICES.forEach(device => {
    const card = filterUICards[device];
    if (!card) return;
    card.root.style.display = filterState[device].isActive ? 'flex' : 'none';
  });
}

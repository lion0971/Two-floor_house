...
setTimeout(() => {
  ...
  setTimeout(showTapPrompt, 50); // 150 → 50
...
Object.assign(tapPromptWelcome.style, {
  ...
  opacity: '0',
  transition: 'opacity 3.4s linear', // ⚡ 1.6s → 1.1s，比原本 0.8s 稍長，兼顧緩衝效果又不會太晚才顯現
});
...
function showTapPrompt() {
  ...
  requestAnimationFrame(() => {
    ...
    tapPromptWelcome.style.opacity = '1'; // ⚡ 文字立刻開始淡入，不用延遲
  });

  // ⚡ 葉子延後一段時間才開始飄入，讓文字先被看到

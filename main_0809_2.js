...
// 調這個數字即可，不用動其他邏輯。
const FILTER_CARD_HEIGHT_OFFSET = 0.4;
...
function createFilterCard(device) {
  // ⚡ 這裡刻意分成三層，職責各自獨立、互不干擾：
  // 1. wrapper：CSS2DRenderer 每一幀直接覆寫它的 transform，用來把整組卡片
  //    定位在 3D 世界座標投影到螢幕上的位置（裝置正上方）。
  // 2. offsetWrapper：當這張卡片跟其它卡片視覺上重疊時，用來做「水平避讓」
  //    位移（translateX），這個位移是我們自己算的，跟展開動畫、3D定位分開。
  // 3. root：實際看到的卡片外觀，展開/淡入動畫（scale + opacity）只作用在這一層。
  ...
  Object.assign(root.style, {
    ...
    fontSize: '13px',
    minWidth: '210px',
    // ⚡ 從中間展開：transformOrigin 設為正中心，展開動畫（縮放+淡入）
    // 都是以卡片自己的中心點為基準，不做位移/跳起，單純由小放大。
    transformOrigin: 'center center',
    position: 'absolute',
    left: '50%',
    bottom: '0',
    marginLeft: '-105px', // 抵銷 minWidth 210px 的一半，讓卡片橫向置中對齊錠點（裝置正上方那個點）
    marginBottom: '10px', // 卡片底部跟錠點留一點垂直間距，不會貼著裝置
    opacity: '0',
    transform: 'scale(0.4)',
  ...
  // ⚡ 注意：這裡不 appendChild 到 document.body，而是留給
  // attachFilterCardsToScene() 用 CSS2DObject 掛進 3D 場景圖，
  // 由 CSS2DRenderer 每一幀自動算好螢幕座標、自動搬進它自己的 DOM 容器。

  filterUICards[device] = { wrapper, offsetWrapper, root, cssObject: null, fg, percentText, hourLine, hideTimer: null };
}
...
function resolveFilterCardOverlaps() {
  ...
  if (visibleDevices.length === 1) {
    // 只剩一張卡片顯示，不需要避讓，直接鬆弛回真實位置
    const card = filterUICards[visibleDevices[0]];
    card.offsetWrapper.style.transition = 'transform 0.25s ease';
    card.offsetWrapper.style.transform = 'translateX(0px)';
    return;
  }

  // 量測前先把所有偏移歸零，確保量到的是「真實（未避讓）」位置，
  // 不然如果先前已經有偏移，這次量測會疊加上次的結果，越算越歪。
  visibleDevices.forEach(d => {
    const w = filterUICards[d].offsetWrapper;
    w.style.transition = 'none';
    w.style.transform = 'translateX(0px)';
  });
  void filterUICards[visibleDevices[0]].offsetWrapper.offsetWidth; // 強制reflow
  ...
  const ITERATIONS = 4; // 卡片數量最多4個，幾輪內就能穩定收斂，不需要太多次
  ...
  rects.forEach(r => {
    ...
    card.offsetWrapper.style.transition = 'transform 0.25s ease';
    card.offsetWrapper.style.transform = `translateX(${r.offset}px)`;
  });
}

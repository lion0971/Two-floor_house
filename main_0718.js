...
if (name.includes('sliding_door')) {
  ...
  // 計算門片寬度，做為平移距離（依你的模型軸向可能要改 y 或 z）
  mesh.geometry.computeBoundingBox();
  ...
  doorAnimations[name] = {
    ...
    direction: 0,
    type: 'sliding',        // ← 新增類型標記
    slideDistance: size.x,  // 平移距離，也可以直接寫死一個數字，例如 0.9
    startX: mesh.position.x, // 記錄初始位置，平移時以此為基準
  };
...
function animate() {
  ...
  if (anim.type === 'sliding') {
    // 沿 mesh 本地 X 軸平移；往左開所以用減號，往右開就改成 +
    anim.mesh.position.x = anim.startX - eased * anim.slideDistance;
  } else {

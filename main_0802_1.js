...
const WATER_EXIT_JUMP = 0.35;        // 剛出水那一刻，比地面高度多躍起的量（先躍起再落地）數字越大 → 躍起越高
...
/**
 * 共用的多高度碰撞判斷：對 targetObjects 用 COLLISION_HEIGHT_OFFSETS
...
function resolveCollisionSlide(moveVelocity, targetObjects) {
  ...
  for (const yOffset of COLLISION_HEIGHT_OFFSETS) {
    // 原本：camera.position.clone() 每個高度都會 new 一個 Vector3
...
loader.load(CONFIG.MODELS.BUILDING, (gltf) => {
  ...
  gltf.scene.traverse((mesh) => {
    ...
    if (name !== 'floor_pool_water' &&
        (name.includes('wall') || name.includes('floor') || name.includes('door') || name.includes('stair'))) {
    ...
    // ⚡ 如果這個名稱已經被上面的滑門邏輯登記過（sliding_door / sliding_up_door），
...
renderer.domElement.addEventListener('click', () => {
  ...
  if (doorIntersects.length) {
    ...
    while (obj && !obj.name.toLowerCase().includes('sliding_door') && !obj.name.toLowerCase().includes('sliding_up_door') && !obj.name.toLowerCase().includes('door_')) {
      obj = obj.parent;
    }

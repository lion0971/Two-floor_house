...
function updateStaircase(delta) {
  ...
  console.log('RAW moveForward:', moveForward, 'moveBackward:', moveBackward); // ← 加這行在最上面
  ...
  if (!isOnStairRail && dist <= STAIRCASE.radius + STAIRCASE.radiusMargin) {
    ...
    // 依目前高度判斷比較接近底層還是頂層（避免從上面樓層走近時被誤判成從0開始）
  ...
  if (!isOnStairRail) return; // 不在樓梯範圍，交給一般走路邏輯

  // ── 軌道模式：W/S 直接控制上下樓，不需要自己繞圈 ──
  const inputDir = Number(moveForward) - Number(moveBackward); // W=+1 上樓，S=-1 下樓（如果方向反了就對調）
  stairHeight += inputDir * STAIRCASE.climbSpeed * delta;

  // 走到頂/到底且繼續往同方向走 → 離開軌道，交還一般走路控制
  ...
  console.log('inputDir:', inputDir, 'stairHeight:', stairHeight.toFixed(2), 'isOnStairRail:', isOnStairRail);
}
...
loader.load(CONFIG.MODELS.BUILDING, (gltf) => {
  ...
  gltf.scene.traverse((mesh) => {
    ...
    // 碰撞
    if (name.includes('wall') || name.includes('floor') || name.includes('door') || name.includes('pillar')) {
...
function toggleXRayMode(enable) {
  ...
  flowingPipes.forEach((p) => {
    ...
    if (!p.active) {
      p.mesh.material.opacity = enable ? 0.45 : 0.12;
    }
  });
}
